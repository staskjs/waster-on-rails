module Api
  class WorkTimeController < ApplicationController
    ...
    def update
      ...
      begin
        time_in = Time.parse(attributes[:time_in])
      rescue => ex
        render json: {error: 'errors.time_in.wrong'}, status: 406 and return
      end
      time_in = work_time.time_in.change(hour: time_in.hour, min: time_in.min)
      attributes[:time_in] = time_in

      begin
        time_out = Time.parse(attributes[:time_out])
      rescue
        render json: {error: 'errors.time_out.wrong'}, status: 406 and return
      end
